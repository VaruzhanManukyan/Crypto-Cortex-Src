...
const DecryptionForm = React.memo(({ setStateValue, setStateKey, stateValue, stateKey, cipher, selectCipher, sendValueForEncryption, result }) => {
    let validatorForCesar = cesarKeyIsNumber(cipher, stateKey, setStateKey);
    const [stateIcon, setStateIcon] = useState("reject");
    ...
    return (
        <form className="encryption-block-main">
            <h1 className="title-styles">Decryption</h1>
            ...
            <Field component={keyInput} placeholder="key" onChangeKey={event => setStateKey(event.target.value)} value={stateKey} name="key" validate={[validatorForCesar]} />
            <div className="style-btn-box">
                <div style={{ cursor: "pointer" }} onClick={sendValueForEncryption} className="btn-box-styles">
                    <a style={{ textDecoration: "none" }} className="btn1-styles">Decrypt</a>
                </div>
...
export default reduxForm({ form: "decryptionForm" })(DecryptionForm);